...
def add_call(calls: Any, name: str) -> List[str]:
    """Append a call name to a run's "what just happened" trace, preserving first-occurrence
    order and skipping a repeat. ONE implementation for all three families (standards §1):
    several handlers can run twice for one run — ``callback`` carries no already-completed
    guard, and the flow / company-data poll loops legitimately re-attempt the same call on
    every poll — so an unconditional append writes the same line again. The trace must read as
    what the run DID.
...
def record_call(run: Dict[str, Any], name: str) -> bool:
    """:func:`add_call` against a run dict in place. Returns True when the name was newly added,
    so the caller can persist on that transition."""
    before = run.get("calls") or []
    run["calls"] = add_call(before, name)
    return len(run["calls"]) != len(before)
