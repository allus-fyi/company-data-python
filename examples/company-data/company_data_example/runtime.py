"""Cross-request state for the company-data demo backend (contract v3, config-file model).
...
* ``config/{sid}.json``       - the canonical SDK config file a scenario runs OFF
  (written by ``POST /api/scenarios/{id}/config`` from the browser settings; NOT
  TTL-swept). ``{sid}`` is a filesystem-safe token of the scenario id
  (``companydata:read`` -> ``companydata_read``).
* ``config/{sid}.meta.json``  - demo-only run parameters that are not SDK Config
  fields (the documents target ``share_code``; the ``webhook_id`` routing key).
* ``config/keys/<sha1>.pem``  - the service private-key file(s) a config references
  by path (mode 0600).
* ``runs/{runId}.json``       - one run's accumulated result (values / events /
  docs) + the ``calls`` trace.
* ``webhook-route.json``      - the SINGLE active webhook run ``{webhookId, runId}``.
  A new ``companydata:webhook`` run supersedes it; TTL/Clear of the run drops it.
* ``cache/``                  - the SDK pump's buffer + dead-letter dir
  (``Config.cache_dir`` -> this path), wiped by Clear / the startup wipe.
...
# Only the service key is referenced by every company-data config.
_KEY_FIELDS = ("service_private_key",)

WEBHOOK_SCENARIO = "companydata:webhook"
...
class Runtime:
    ...
    @staticmethod
    def sid(scenario_id: str) -> str:
        """Filesystem-safe token for a scenario id (``companydata:read`` -> ``companydata_read``)."""
        return _SID_RE.sub("_", scenario_id).strip("_")

    def config_path_for(self, scenario_id: str) -> str:
        return os.path.join(self.config_dir, f"{self.sid(scenario_id)}.json")

    def meta_path_for(self, scenario_id: str) -> str:
        return os.path.join(self.config_dir, f"{self.sid(scenario_id)}.meta.json")

    def has_config(self, scenario_id: str) -> bool:
        return os.path.isfile(self.config_path_for(scenario_id))

    def write_config(self, scenario_id: str, config: Dict[str, Any]) -> str:
        ...
        return f".runtime/config/{self.sid(scenario_id)}.json"

    def write_config_meta(self, scenario_id: str, meta: Dict[str, Any]) -> None:
        self.ensure_dirs()
        _atomic_write(self.meta_path_for(scenario_id), _dumps(meta))

    def read_config_meta(self, scenario_id: str) -> Dict[str, Any]:
        return _read_json(self.meta_path_for(scenario_id)) or {}

    def load_config(self, scenario_id: str) -> Dict[str, Any]:
    ...
    def clear_scenario(self, scenario_id: str) -> None:
        """Delete a scenario's run files + config + meta, drop the route (webhook), wipe the
        pump cache, then GC unreferenced key PEMs."""
        for name in _listdir(self.runs_dir):
            if not name.endswith(".json"):
                continue
            path = os.path.join(self.runs_dir, name)
            data = _read_json(path)
            if data and str(data.get("scenario") or "") == scenario_id:
                _unlink(path)
        _unlink(self.config_path_for(scenario_id))
        _unlink(self.meta_path_for(scenario_id))
        if scenario_id == WEBHOOK_SCENARIO:
            self.clear_route()
