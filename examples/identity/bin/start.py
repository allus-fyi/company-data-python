#!/usr/bin/env python3
"""One-command launcher for the Python identity example.
...
own dependencies (the local ``allus-company-data`` SDK, editable, + Authlib for
the OIDC scenarios) on first run, then hands off to ``identity_example`` running
under that venv. A present venv is reused (nothing is reinstalled).
...
def main() -> None:
    ...
    # example package (identity_example) regardless of the caller's cwd.
    env = dict(os.environ, PYTHONPATH=BASE)
    os.execve(py, [py, "-m", "identity_example", *sys.argv[1:]], env)
